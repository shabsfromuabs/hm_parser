  ...
  addContentScripts() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      if (currentTab.url && currentTab.url.startsWith('https://online.ukrsibbank.com/ibank')) {
        ...
        chrome.tabs.executeScript(currentTab.id, { file: 'includes/parser/parse.js' });
      } else if (currentTab.url && currentTab.url.startsWith('https://wise.com/balances/')) {
        chrome.tabs.executeScript(currentTab.id, { file: 'includes/parser/wiseParser.js' });
        chrome.tabs.executeScript(currentTab.id, { file: 'includes/parser/parse.js' });
      } else if (currentTab.url && currentTab.url.startsWith('https://www.bankmillennium.pl/')) {
        chrome.tabs.executeScript(currentTab.id, { file: 'includes/parser/millenniumParser.js' });
        chrome.tabs.executeScript(currentTab.id, { file: 'includes/parser/parse.js' });
