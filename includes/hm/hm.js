class HmUploader {
  ...
  constructor() {
    ...
    this.onShouldExportTransactionChanged =
      this.onShouldExportTransactionChanged.bind(this);
  }
  ...
  preparePage() {
    ...
    this.container = appendHtmlElement(
      "div",
      this.wrapper,
    ...
    chrome.storage.local.get("credentials", (result) => {
      ...
      getToken(credentials.email, credentials.password)
        .then((token) => {
          ...
          getCategories(credentials.email, token)
            .then((categories) => {
            ...
            .catch((e) => {
              console.warn(e);
              alert(`Unable to get categories: ${e.message || e}`);
            });
        })
        .catch((e) => {
          console.warn(e);
  ...
  renderTransactions() {
    ...
    this.transactions.forEach((tr, i) => {
      ...
      const form = appendHtmlElement("form", this.container, rowClass, {
        id: `transaction-row-${i}`,
      });
  ...
  addExportBtn() {
    ...
    const exportBtn = appendHtmlElement(
      "button",
      col,
      "btn btn-success btn-lg",
      { innerText: "Експортувати" }
    );
  ...
  renderTransactionControls(tr, id) {
    ...
    const togglesWrap = appendHtmlElement(
      "div",
      colToggle,
      "btn-group-vertical btn-group-sm btn-group-toggle"
    );
    this.TRANSACTION_TYPES.forEach((type) => {
      const radioBtn = addToggleBtn(
        togglesWrap,
        "transaction_type",
        type,
        tr.type === type,
        this.TRANSACTION_TYPE_TRANSLATES[type]
      ).querySelector("input");
  ...
  onShouldExportTransactionChanged(e) {
    ...
    e.target.parentElement.classList[e.target.checked ? "add" : "remove"](
      "checked"
    );
  ...
  onTransactionTypeChange(e) {
    ...
    this.renderTransactionDetails(
      this.convertTransactionToType(transaction, newTransactionType),
      transactionId
    );
  }

  renderTransactionDetails(tr, id) {
    const { form, colAmount, colSelects, colDescription } =
      this.transactionRows[id];
    // Clear existing HTML
    colAmount.innerHTML = "";
    ...
    // Date
    appendHtmlElement("input", colAmount, "", {
      type: "hidden",
    ...
    addAccountsSelect(
      colSelects,
      "transaction_account",
      tr.accountId || tr.fromAccountId,
      isTransfer ? "d-none" : ""
    );
    addAccountsSelect(
      colSelects,
      "transaction_account_from",
      tr.fromAccountId || tr.accountId,
      !isTransfer ? "d-none" : ""
    );
    addAccountsSelect(
      colSelects,
      "transaction_account_to",
      tr.toAccountId || tr.accountId,
      !isTransfer ? "d-none" : ""
    );
    // Category
    addCategoriesSelect(
    ...
    // Description
    const formGroup = addTextAraFormGroup(
      colDescription,
  ...
  getTransactionDetails(id, typeOverride = null) {
    ...
    const shouldExport = form.querySelector(
      'input[name="should_export_transaction"]'
    ).checked;
  ...
  convertTransactionToType(transaction, type) {
    ...
    const { amount, fromAmount } = transaction;
    const amnt = amount || fromAmount;

    if (type === "expense") {
      return { ...transaction, type, amount: -1 * Math.abs(amnt) };
    } else if (type === "transfer") {
      return {
        ...transaction,
        type,
        fromAmount: Math.abs(amnt),
        toAmount: Math.abs(amnt),
      };
    }
    return { ...transaction, type, amount: Math.abs(amnt) };
  }
